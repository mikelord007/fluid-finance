...
## `FungibleTokenOption` contract interface
...
would need to implement the FungibleTokenOption interface.
...
///
pub contract interface FungibleTokenOption {
    ...
    pub resource interface Provider {
        ...
        pub fun withdraw(amount: UFix64): @Vault {
            post {
                ...
                result.payoutVault.balance == result.balance: "Payout Balance not same as Option Balance in result"
            }
        }
    }
    ...
    pub resource interface Receiver {
        ...
        /// resource doesn't conform with the `FungibleTokenOption.Vault` resource.
        ///
        ...
        pub fun getSupportedVaultTypes(): {Type: Bool} {
            ...
            // only get returned when the parent resource conforms with `FungibleTokenOption.Vault`. 
            if self.getType().isSubtype(of: Type<@FungibleTokenOption.Vault>()) {
                return {self.getType(): true}
            } else {
                // Return an empty dictionary as the default value for resource who don't
                // implement `FungibleTokenOption.Vault`, such as `FungibleTokenOptionSwitchboard`, `TokenForwarder` etc.
                return {}
            }
        }
    }
    ...
    /// every Fungible Token contract that implements the FungibleTokenOption interface
    ...
    pub resource Vault: Provider, Receiver, Balance {
        ...
        pub fun deposit(from: @Vault) {
            ...
            post {
                ...
                self.balance == self.payoutVault.balance: "Payout not same as Option"
            }
        }
    }
    ...
    pub fun createEmptyVault(): @Vault {
        post {
            ...
            result.payoutVault.balance == result.balance
        }
    }
}